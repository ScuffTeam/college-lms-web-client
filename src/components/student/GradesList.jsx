...
export function GradesList() {
    ...
    const calculateAverage = (grades) => {
        ...
        const validGrades = grades.filter(grade => grade && typeof grade.value === 'number');
                    ...
                    {dayGrades.map((grade, index) => (
                        <div class={styles['grade-item']} key={index}>
                            ...
                            <div class={styles['grade-info']}>
                                ...
                                <div class={styles['grade-value']} style={{ color: getGradeColor(grade.value) }}>
                                    {grade.value}
                                </div>
                            </div>
                        </div>
            ...
            <div class={styles['subject-grades']}>
                ...
                <div class={styles['grades-list']}>
                    {subjectGrades.grades.map((grade, index) => (
                        <div class={styles['grade-item']} key={index}>
                            <div class={styles['grade-value']} style={{ color: getGradeColor(grade?.value || 0) }}>
                                {grade?.value || 'Н/Д'}
                            </div>
                            <div class={styles['grade-info']}>
                                <div class={styles['grade-type']}>{grade?.type || 'Не указано'}</div>
                                <div class={styles['grade-date']}>{grade?.date ? formatDate(grade.date) : 'Не указано'}</div>
                            </div>
                        </div>
                    ))}
                </div>
            ...
            <div class={styles['section-header']}>
                ...
                <div class={styles['grades-summary']}>
                    <div class={styles['summary-item']}>
                        ...
                        <span>Средний балл: {calculateAverage(grades().flatMap(g => g?.grades || []))}</span>
                    </div>
                </div>
            </div>
                ...
                <>
                    ...
                    <div class={styles['all-grades-section']}>
                        ...
                        <div class={styles['grades-grid']}>
                            {grades().map(grade => grade?.subject && renderSubjectGrades(grade.subject))}
                        </div>
                    </div>
                </>
            )}
