            ...
            {loading() ? (
                ...
                <Show when={selectedGroup()}>
                    ...
                    <div class={styles.weekNavigation}>
                         <button onClick={goToPreviousWeek} class={styles.navButton}>&lt;</button>
                         <div class={styles.currentMonthYear}>
                             {getMonthYear(currentWeekDays()[0])} - {getMonthYear(currentWeekDays()[6])}
                         </div>
                         <button onClick={goToNextWeek} class={styles.navButton}>&gt;</button>
                    </div>
                    ...
                    <div class={styles.calendarGrid}>
                        ...
                        <For each={currentWeekDays()}>
                            {(date) => {
                                ...
                                return (
                                    <div 
                                        class={`${styles.calendarDay} ${hasLessons ? styles.hasLessons : ''}`}
                                        onClick={() => openModal(date)}
                                        key={date.toDateString()}
                                    >
                                        <div class={styles.dayNumber}>{getDayNumber(date)}</div>
                                        <div class={styles.dayNameShort}>{getDayName(date).substring(0, 2).toUpperCase()}</div>
                                        {hasLessons && (
                                            <div class={styles.lessonIndicator}></div>
                                        )}
                                    </div>
                                );
                            }}
            ...
            <Show when={showModal() && selectedDaySchedule()}>
                <div class={styles.modalBackdrop} onClick={closeModal}>
                    <div class={styles.modalContent} onClick={(e) => e.stopPropagation()}>
                         <h3>Расписание на {selectedDaySchedule().date.toLocaleDateString('ru-RU', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</h3>
                        <Show 
                            when={selectedDaySchedule().lessons.length > 0}
                            fallback={<p class={styles.emptySlot}>На этот день уроков нет.</p>}
                        >
                            <ul class={styles.lessonList}>
                                <For each={selectedDaySchedule().lessons}>
                                    {(lesson, index) => (
                                        <li class={styles.lessonItem} key={lesson.id || index()}>
                                            <span class={styles.lessonTime}>{lesson.time}:</span> {lesson.subject}
                                        </li>
                                    )}
                                </For>
                            </ul>
                        </Show>
                        <button onClick={closeModal} class={styles.closeButton}>Закрыть</button>
                    </div>
                </div>
            </Show>
        </div>
